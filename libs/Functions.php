...
class Functions {
	...
	public function showProfile($db, $memID){
		$query = "SELECT email, identity, age, gender, occupation, profilepicture FROM memberProfile WHERE memberID=?;";
...
//----------------------LIST OF FUNCTIONS2-----------------------------------//
	...
	public function getSessionID($db,$email){
        ...
        $_SESSION['login_string']	= hash('sha512', "google".$user_browser);
	...
	public function addMember($db, $email, $identity, $age, $gender, $occupation,$profilepicture, $memberlocation, $bio){
		$res = self::memberExists($db, $email);
		if ($res == 1){
			return (1);
		}
		$query="INSERT INTO memberProfile(email,identity,age,gender,occupation,profilepicture,memberlocation,bio) VALUES(?,?,?,?,?,?,?,?);";
		$db->query($query);
		$db->bind(1,$email);
		$db->bind(2,$identity);
		$db->bind(3,$age);
		...
		$db->bind(6,$profilepicture);
		$db->bind(7,$memberlocation);
		$db->bind(8,$bio);
	...
	public function updateEvent($db, $actionID, $eventName, $startDate, $endDate, $description, $location, $coverPicture, $minimumMembers=5){
 		$query = "UPDATE actions SET(creatorid, eventname, startdate, enddate, description, location, coverpicture, minattend) = (?,?,?,?,?,?,?,?) WHERE actionID=?;";
		$db->query($query);
		$db->bind(1,$_SESSION["memberID"]);
		$db->bind(2,$eventName);
		$db->bind(3,$startDate);
	...
	public function addRating($db, $memberID,$aboutMemberID, $rating, $comment=""){
		$query="SELECT foo1.actionID FROM participants AS foo1 CROSS JOIN(SELECT actionID,memberID FROM participants) AS foo WHERE foo.memberID=? and foo1.memberID=? and foo1.actionID=foo.actionID;";
		$db->query($query);
		$db->bind(1,$memberID);
		$db->bind(2,$aboutMemberID);
		$db->execute();
		$res = $db->fetchOne();
		if (empty($res)){
			return(1);
...
/*
	...
	public function removeFriend($db, $friendEmail){
		$query = "SELECT memberID FROM memberProfile WHERE email=?;";
		$db->query($query);
		$db->bind(1,$friendEmail);
		$db->execute();
		...
		$query="DELETE FROM friends WHERE friend1=? and friend2=?;";
		$db->query($query);
		$db->bind(1,$_SESSION["memberID"]);
		$db->bind(2,$friendID["memberid"]);
		$db->execute();
	...
	public function getFriendsWithInfo($db, $memberID){
		$query = "SELECT friend2 AS memberID FROM friends WHERE friend1=?";
		$query2 = "SELECT identity, email, occupation, memberID, profilepicture FROM (" . $query . ") AS query NATURAL JOIN memberprofile;";
	...
	public function getUpcomingEvents2($db, $memberID){
		$query = "SELECT eventname, location, startdate, description FROM actions NATURAL JOIN participants WHERE memberID=? ORDER BY startdate LIMIT 2;";
	...
	public function getUpcomingForFriends($db, $memberID){
		$friends = self::getMemberFriends($db, $memberID);

		$eventList = array();

		for ($i=0; $i < count($friends) ; $i++) {
			$event = self::getUpcomingEvents2($db, $friends[$i]['friend2']);
		...
		foreach ($eventList as $key => $row) {
    		$dates[$key]  = $row['startdate']; 
  			// of course, replace 0 with whatever is the date field's index
		}
		//Make the output unique
		array_multisort($dates, SORT_ASC, $eventList);

		$res = array_unique($eventList);

		$duplicates = array_diff_key( $eventList, $res );
		return ($duplicates);
	}


	public function makeHangout($db,$actionID,$hangouturl){
		$query="INSERT INTO hangout(creatorID, actionID, hangouturl) VALUES(?,?,?);";
		$db->query($query);
		$db->bind(1,$_SESSION["memberID"]);
		$db->bind(2,$actionID);
		$db->bind(3,$hangouturl);
		$db->execute();
	}

	public function removeHangout($db,$actionID,$hangouturl){
		$query="DELETE FROM hangout WHERE creatorID=? and actionID=? and hangouturl=?;";
		$db->query($query);
		$db->bind(1,$_SESSION["memberID"]);
		$db->bind(2,$actionID);
		$db->bind(3,$hangouturl);
